...
bool AudioUploadService::start() {
    ...
    m_queue = xQueueCreate(16, sizeof(UploadJob));
    if (m_queue == nullptr) {
    ...
    ESP_LOGI(kTag, "upload worker started");
    return true;
}
...
bool AudioUploadService::sendChunkJob(UploadJob& job) {
    ...
    if (!protocol.buildSpeechChunk(
            ...
            m_postBody,
            sizeof(m_postBody))) {
        ESP_LOGW(kTag, "chunk JSON too large seq=%lu", static_cast<unsigned long>(job.chunkSeq));
        freeJobPcm(job);
        return false;
    }

    const bool ok = postJson(m_callbacks.speechUrl, m_postBody);
    freeJobPcm(job);
    ...
    if ((job.chunkSeq % 25U) == 0U) {
        ESP_LOGI(
            kTag,
            "chunk uploaded id=%s seq=%lu ok=%lu fail=%lu",
            job.utteranceId,
            static_cast<unsigned long>(job.chunkSeq),
            static_cast<unsigned long>(m_postsOk),
            static_cast<unsigned long>(m_postsFail));
    }
    return true;
}
...
bool AudioUploadService::sendFinalizeJob(const UploadJob& job) {
    ...
    const bool ok = postJson(m_callbacks.speechFinalizeUrl, finalizeBody);
    if (ok) {
        ESP_LOGI(
            kTag,
...
bool AudioUploadService::uploadChunk(const char* utteranceId, const char* sessionId, const uint8_t* pcm, size_t pcmLen) {
    if (!m_running || !isConfigured() || pcm == nullptr || pcmLen == 0 || m_queue == nullptr) {
        return false;
    }
    ...
    if (xQueueSend(static_cast<QueueHandle_t>(m_queue), &job, 0) != pdTRUE) {
        freeJobPcm(job);
